...
export default function BasePage({ params }: { params: Promise<{ baseId: string }> }) {
  ...
  function handleAddTable() {
    ...
    createTable.mutate(
      { baseId, name: newTableName.trim() },
      { onSuccess: (table) => setActiveTableId(table.id) }
    );
    setNewTableName("");
  ...
  return (
    <div className="min-h-screen bg-[#0e0e10] text-white flex flex-col" style={{ fontFamily: "'DM Mono', monospace" }}>
        ...
        <div className="flex items-center">
            ...
            return (
              ...
              >
                {isRenaming ? (
                  <input
                    autoFocus
                    className="mx-2 my-2 bg-[#1e1e28] border border-[#5b6af7] rounded px-2 py-0.5 text-sm outline-none w-32"
          ...
          {addingTable ? (
            <div className="flex items-center gap-1 ml-1 py-2">
              <input
                autoFocus
                className="bg-[#1e1e28] border border-[#5b6af7] rounded px-2 py-0.5 text-sm outline-none w-28"
      ...
      {/* Content */}
      <div className={`flex-1 ${view === "kanban" ? "p-6 overflow-hidden" : "overflow-auto"}`}>
        {currentTableId ? (
          view === "grid"
