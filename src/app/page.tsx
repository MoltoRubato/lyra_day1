...
type PageView = "home" | "starred" | "workspaces" | string; // string = workspaceId
...
type WsItem = { id: string; name: string };
type WsFull = {
...
const BASE_PALETTE = [
  "#166254","#1c7ed6","#862e9c","#e8590c",
  "#0ca678","#4263eb","#c92a2a","#5c7cfa",
  "#0891b2","#d97706",
];
function baseColor(id: string): string {
...
function timeAgo(date: Date | null | undefined): string {
  ...
  if (s < 3600) { const m = Math.floor(s / 60);  return `Opened ${m} minute${m !== 1 ? "s" : ""} ago`; }
  if (s < 86400){ const h = Math.floor(s / 3600); return `Opened ${h} hour${h !== 1 ? "s" : ""} ago`; }
  const dy = Math.floor(s / 86400);
  return `Opened ${dy} day${dy !== 1 ? "s" : ""} ago`;
}

// ─── Small shared UI ──────────────────────────────────────────────────────────

function BaseIcon({ name, id, size = 28 }: { name: string; id: string; size?: number }) {
  return (
    <div className="rounded flex items-center justify-center text-white font-semibold flex-shrink-0"
      style={{ width: size, height: size, fontSize: Math.round(size * 0.42), background: baseColor(id) }}>
      {name[0]?.toUpperCase() ?? "?"}
    </div>
  );
}
...
function NavBtn({
  icon, label, active, collapsed, onClick, children,
}: {
    ...
    <div>
      <button onClick={onClick}
        className={`w-full flex items-center gap-2.5 px-2.5 py-[7px] rounded-md text-[13px] transition-colors leading-tight ${
          active ? "bg-[#f2f4f8] text-[#172b4d] font-semibold" : "text-[#444] hover:bg-[#f5f5f4] hover:text-[#172b4d]"
...
const StarIco = () => (
  <svg width="15" height="15" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round">
    <path d="M8 2L9.8 6.2L14 6.5L10.8 9.3L11.8 13.5L8 11.2L4.2 13.5L5.2 9.3L2 6.5L6.2 6.2L8 2Z"/>
  </svg>
);
const WsIco = () => (
...
const PencilIco = ({ size = 11 }: { size?: number }) => (
...
const MoveIco = ({ size = 11 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.3">
    <rect x="1.5" y="2" width="4" height="8" rx="0.5"/><rect x="6.5" y="2" width="4" height="8" rx="0.5"/>
  </svg>
);
const TrashIco = ({ size = 11 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.3">
...
export default function HomePage() {
  ...
  const { data: workspaces = [], isLoading: wsLoading }     = api.workspace.getAll.useQuery();
  const isLoading = basesLoading || wsLoading;

  // ── Mutations — base ───────────────────────────────────────────────────────
  const createBase = api.base.create.useMutation({
    onSuccess: () => { void utils.base.getAll.invalidate(); void utils.workspace.getAll.invalidate(); },
  });
  const renameBase = api.base.rename.useMutation({
    onMutate: ({ id, name }) => utils.base.getAll.setData(undefined, (p) => p?.map((b) => b.id === id ? { ...b, name } : b)),
    onSettled: () => void utils.base.getAll.invalidate(),
  });
  const deleteBase = api.base.delete.useMutation({
    onMutate: ({ id }) => utils.base.getAll.setData(undefined, (p) => p?.filter((b) => b.id !== id)),
    onSettled: () => { void utils.base.getAll.invalidate(); void utils.workspace.getAll.invalidate(); },
  });
  const toggleBaseStar = api.base.toggleStar.useMutation({
    onMutate: ({ id, starred }) => utils.base.getAll.setData(undefined, (p) => p?.map((b) => b.id === id ? { ...b, starred } : b)),
    onSettled: () => void utils.base.getAll.invalidate(),
  });
  ...
  const renameWs = api.workspace.rename.useMutation({
    onMutate: ({ id, name }) => utils.workspace.getAll.setData(undefined, (p) => p?.map((w) => w.id === id ? { ...w, name } : w)),
  ...
  const deleteWs = api.workspace.delete.useMutation({
    onMutate: ({ id }) => utils.workspace.getAll.setData(undefined, (p) => p?.filter((w) => w.id !== id)),
    onSettled: () => { void utils.workspace.getAll.invalidate(); void utils.base.getAll.invalidate(); },
  });
  const toggleWsStar = api.workspace.toggleStar.useMutation({
    onMutate: ({ id, starred }) => utils.workspace.getAll.setData(undefined, (p) => p?.map((w) => w.id === id ? { ...w, starred } : w)),
    onSettled: () => void utils.workspace.getAll.invalidate(),
  });
  ...
  const filteredWs = useMemo(() => {
    if (!search.trim()) return workspaces as WsFull[];
    const q = search.toLowerCase();
    return (workspaces as WsFull[]).filter((w) => w.name.toLowerCase().includes(q));
  }, [workspaces, search]);
  ...
  function open(m: ModalState) {
    setModal(m);
    if (!m) return;
    if (m.kind === "createBase")      { setNewName(""); }
  ...
  return (
    <div className="min-h-screen flex bg-[#f9fafb]"
      ...
      {/* ══ SIDEBAR ══════════════════════════════════════════════════════════ */}
      <aside className={`fixed top-0 left-0 bottom-0 bg-white border-r border-[#e0e0e0] flex flex-col transition-all duration-200 z-20 overflow-hidden flex-shrink-0 ${sidebarOpen ? "w-[232px]" : "w-[48px]"}`}>
              ...
              {(workspaces as WsFull[]).map((ws) => (
                <button key={ws.id} onClick={() => setPage(ws.id)}
                  className={`w-full flex items-center gap-2 px-2 py-[6px] rounded-md text-xs transition-colors text-left ${
                    page === ws.id ? "bg-[#f9fafb] text-[#172b4d] font-semibold" : "text-[#666] hover:bg-[#f5f5f4] hover:text-[#172b4d]"
                  }`}>
                  <span className="flex-1 truncate">{ws.name}</span>
                  {ws.starred && <span className="text-yellow-400 text-[10px] flex-shrink-0">★</span>}
                </button>
              ))}
        ...
        {/* Create button */}
        <div className={`flex-shrink-0 border-t border-[#e0e0e0] p-3 ${!sidebarOpen ? "p-2" : ""}`}>
      ...
      <main className={`flex-1 flex flex-col min-h-screen transition-all duration-200 ${sidebarOpen ? "ml-[232px]" : "ml-[48px]"}`}>
        ...
        {/* Top bar */}
        <div className="h-[52px] bg-[#ffffff] border-b border-[#e0e0e0] flex items-center px-6 gap-3 flex-shrink-0">
          {/* Search bar */}
          <div className="flex-1 flex justify-center">
            <div className="flex items-center gap-2 bg-white border border-[#d8d8d8] rounded-full px-3.5 py-1.5 w-full max-w-[420px] shadow-sm">
              ...
              <input
                className="flex-1 bg-transparent outline-none text-[13px] text-[#172b4d] placeholder-[#aaa] min-w-0"
                placeholder="Search..."
                value={search}
            ...
            <>
              ...
              <div className="flex items-center justify-between mb-0">
                ...
                <div className="flex items-center gap-0.5">
                  <button onClick={() => setDispMode("list")} title="View items in a list"
                    className={`p-1.5 rounded transition-colors ${dispMode === "list" ? "text-[#172b4d] bg-[#e8e8e8]" : "text-[#999] hover:text-[#555]"}`}>
                    <ListIco/>
                  </button>
                  <button onClick={() => setDispMode("grid")} title="View items in a grid"
              ...
              {/* Starred workspaces section (only on starred page) */}
                ...
                <div className="flex items-start justify-between mt-3 mb-4">
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <button onClick={() => toggleWsStar.mutate({ id: currentWorkspace.id, starred: !currentWorkspace.starred })}
                      className={`text-base ${currentWorkspace.starred ? "text-yellow-400" : "text-[#ccc] hover:text-yellow-400"} transition-colors`}>★</button>
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/20 flex items-center justify-center z-50 backdrop-blur-[1px]"
          onClick={close}>
          <div className="bg-white rounded-xl border border-[#e0e0e0] shadow-2xl w-full max-w-sm p-6"
...
function BaseListView({ bases, showWorkspace, onRename, onDelete, onStar, onMove }: {
  ...
  const cols = showWorkspace
    ? "grid-cols-[1fr_200px_180px_100px]"
  ...
  return (
    <div>
      ...
      {bases.map((base) => (
        <div key={base.id}
          className={`group grid items-center px-1 py-2 border-b border-[#ebebeb] hover:bg-white transition-colors -mx-1 px-1 ${cols}`}>
          {/* Name column */}
          <div className="flex items-center gap-2.5 min-w-0 pr-4">
            <BaseIcon name={base.name} id={base.id} size={26}/>
          ...
          {/* Workspace */}
          {showWorkspace && (
            <span className="text-xs text-[#666]">{base.workspace?.name ?? "—"}</span>
          )}
          ...
          <div className="flex items-center gap-0 opacity-0 group-hover:opacity-100 transition-opacity justify-end">
            ...
            <ActionBtn title="Move to workspace" onClick={() => onMove(base)}><MoveIco/></ActionBtn>
    ...
    <div className="pt-4 grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))" }}>
      {bases.map((base) => (
        <div key={base.id}
          ...
          {/* Top color strip */}
          <div className="h-[6px]" style={{ background: baseColor(base.id) }}/>
          <div className="p-4">
            <div className="flex items-start gap-3 mb-3">
              <BaseIcon name={base.name} id={base.id} size={34}/>
              <div className="flex-1 min-w-0 pt-0.5">
                ...
                {base.workspace && (
                  <p className="text-[11px] text-[#888] truncate mt-0.5">{base.workspace.name}</p>
                )}
              </div>
              <button onClick={() => onStar(base)}
            ...
            <div className="ml-[44px] space-y-0.5">
              {ws.bases.slice(0, 6).map((base) => (
                <div key={base.id} className="group/row flex items-center gap-2.5 py-1.5 px-1 rounded-md hover:bg-white transition-colors -mx-1">
                  <BaseIcon name={base.name} id={base.id} size={22}/>
                  <Link href={`/base/${base.id}`}
                  ...
                  <div className="flex items-center gap-0 opacity-0 group-hover/row:opacity-100 transition-opacity flex-shrink-0">
                    <button onClick={() => onStarBase(base)}
                      className={`px-1 text-sm ${base.starred ? "text-yellow-400" : "text-[#ddd] hover:text-yellow-400"} transition-colors`}>★</button>
                    <ActionBtn title="Rename" onClick={() => onRenameBase(base)}><PencilIco size={10}/></ActionBtn>
                    <ActionBtn title="Move to different workspace" onClick={() => onMoveBase(base)}><MoveIco size={10}/></ActionBtn>
          ...
          <div className="space-y-0.5">
            {unassigned.map((base) => (
              <div key={base.id} className="group/row flex items-center gap-2.5 py-1.5 px-1 rounded-md hover:bg-white transition-colors -mx-1">
                ...
                <span className="text-[11px] text-[#bbb] flex-shrink-0">{timeAgo(base.lastOpenedAt)}</span>
                <div className="flex items-center gap-0 opacity-0 group-hover/row:opacity-100 transition-opacity flex-shrink-0">
                  <ActionBtn title="Move to workspace" onClick={() => onMoveBase(base)}><MoveIco size={10}/></ActionBtn>
                  <ActionBtn title="Delete" danger onClick={() => onDeleteBase(base.id)}><TrashIco size={10}/></ActionBtn>
                </div>
              </div>
            ))}
